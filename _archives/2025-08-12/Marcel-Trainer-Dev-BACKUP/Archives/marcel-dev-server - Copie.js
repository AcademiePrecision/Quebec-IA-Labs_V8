...
app.post("/test-marcel-response", async (req, res) => {
  ...
  if (ContextAnalyzer) {
    ...
    // Analyser l'input
    const extracted = analyzer.analyzeUserInput(userInput, session);
    ...
    console.log("📊 État session:", {
      ...
      questionsAsked: session.questionsAsked ? Array.from(session.questionsAsked) : [],
    });

    // Créer une copie propre sans références circulaires
    const cleanExtractedInfo = {};
    if (session.extractedInfo) {
      Object.keys(session.extractedInfo).forEach(key => {
        if (typeof session.extractedInfo[key] !== 'object') {
          cleanExtractedInfo[key] = session.extractedInfo[key];
        }
      });
    }
...
${JSON.stringify(cleanExtractedInfo, null, 2)}
...
RÈGLES IMPORTANTES:
1. Si tu reconnais le client, salue-le par son nom de façon chaleureuse
2. Suggère son barbier préféré s'il en a un
3. Tiens compte de ses préférences (matin/soir, services habituels)
4. Ne redemande JAMAIS ce qui est déjà collecté
5. Sois personnalisé et chaleureux
6. Maximum 2 phrases courtes en français québécois
