...
let MarcelDataSystem;
try {
  MarcelDataSystem = require("./relationship-data");
  console.log("✅ Système de relations Client-Barbier-Salon V8.1 chargé!");
  console.log(`   📊 ${MarcelDataSystem.clients.length} clients en base`);
  console.log(`   💈 ${Object.keys(MarcelDataSystem.salons).length} salons connectés`);
} catch (error) {
...
async function generateMarcelResponse(userInput, sessionId = null, phoneNumber = null) {
  ...
  // 🎯 NOUVEAU: Utiliser le système de relations amélioré
  let clientInfo = null;
  ...
  if (phoneNumber && MarcelDataSystem) {
    ...
    if (intelligence.isKnownClient) {
      clientInfo = intelligence.client;
      isKnownNumber = true;
      clientContext = `\n🎯 CLIENT RECONNU:
...
- Salon habituel: ${intelligence.client.preferredSalon}
...
- Préfère: ${intelligence.client.preferences.timePreference}`;
  ...
  } else if (phoneNumber && knownClients.has(phoneNumber)) {
    // Fallback vers ancien système si nouveau pas disponible
